...
template <class T>
bool poleCmp(Point<T> a, Point<T> b) {
    if (sgn(a) != sgn(b)) {
        return sgn(a) == 1;
    } else {
        return cross(a, b) > 0;
...
void solve() {
    ...
    sort(p.begin(), p.end(), [&](int i, int j) { return poleCmp(a[i], a[j]); });
    Frac<__int128> ans(1e18, 1);
    int ansi = -1, ansj = -1, S = -1;
    for (int i = 0; i < n; i++) {
        int j = (i + 1) % n;
        int x = p[i], y = p[j];
        Frac<__int128> res(dot(a[x], a[y]) * dot(a[x], a[y]), square(a[x]) * square(a[y]));
        int s = dot(a[x], a[y]) > 0 ? 1 : -1;
        if (s > S || (s == S && (res - ans) * s > 0)) {
            ans = res;
            S = s;
            ansi = x;
            ansj = y;
