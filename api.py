...
async def change_refer(request: Request):
    ...
    inp_root = data.get('inp_root')
    save_root_vocal = data.get('save_root_vocal')
    paths = data.get('paths')  # 假设paths是一个数组
    save_root_ins = data.get('save_root_ins')
    agg = data.get('agg')
    format0 = data.get('format0')

    result_generator = tools.webui.uvr(model_name, inp_root, save_root_vocal, paths, save_root_ins, agg, format0)
    results = list(result_generator)[-1]
